...
- (void)viewDidLoad {
    ...
    self.jsManager.checkAPILegal = ^BOOL(NSString *apiName) {
        return [weakSelf checkAPILegal:apiName];
    };
    [self.jsManager installPluginJS:@[@"Base",@"Console"]];
    
    [self.view addSubview:self.webView];
    
    // demo: http://192.168.1.251:8282/fed/app-web-sdk/
    [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://192.168.1.114:8080/cc-sdk/dist/?env=2&debug=1"]]];
        ...
        if (self.jsManager.notificationList.count > 0) {
            for (int index = 0; index < 100; index++) {
                [self.jsManager.webViewBridge callHandler:@"device.notification.listeningEvent"
                                                     data:[KKWebViewJavaScriptManager createSuccessResponseData:@(index)]];
            }
        }
    }
}
