...
NSString *const KKSSZipArchiveErrorDomain = @"KKSSZipArchiveErrorDomain";

#define CHUNK 16384

@interface KKSSZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end

@implementation KKSSZipArchive
{
    NSString *_path;
    NSString *_filename;
    zipFile _zip;
...
+ (BOOL)isPasswordValidForArchiveAtPath:(NSString *)path password:(NSString *)pw error:(NSError **)error {
    ...
    if (zip == NULL) {
        if (error) {
            *error = [NSError errorWithDomain:KKSSZipArchiveErrorDomain
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(nullable id<KKSSZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(nullable NSString *)password error:(NSError **)error delegate:(nullable id<KKSSZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
        ...
        progressHandler:(void (^)(NSString *entry, kk_unz_file_info zipInfo, long entryNumber, long total))progressHandler
     ...
     preserveAttributes:(BOOL)preserveAttributes
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError * *)error
               delegate:(nullable id<KKSSZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
     ...
     preserveAttributes:(BOOL)preserveAttributes
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError **)error
               delegate:(id<KKSSZipArchiveDelegate>)delegate
        progressHandler:(void (^)(NSString *entry, kk_unz_file_info zipInfo, long entryNumber, long total))progressHandler
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError * __nullable error))completionHandler
{
    ...
    if ([path length] == 0 || [destination length] == 0)
    {
        ...
        NSError *err = [NSError errorWithDomain:KKSSZipArchiveErrorDomain code:SSZipArchiveErrorCodeInvalidArguments userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:KKSSZipArchiveErrorDomain code:SSZipArchiveErrorCodeFailedOpenZipFile userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    kk_unz_global_info  configInfo = {0ul, 0ul};
    unzGetGlobalInfo(zip, &configInfo);
    
    // Begin unzipping
    if (unzGoToFirstFile(zip) != UNZ_OK)
    {
        NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"failed to open first file in zip file"};
        NSError *err = [NSError errorWithDomain:KKSSZipArchiveErrorDomain code:SSZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if ([delegate respondsToSelector:@selector(zipArchiveWillUnzipArchiveAtPath:zipInfo:)]) {
        [delegate zipArchiveWillUnzipArchiveAtPath:path zipInfo:configInfo];
    }
    ...
    do {
        @autoreleasepool {
            ...
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:@"KKSSZipArchiveErrorDomain" code:SSZipArchiveErrorCodeFailedOpenFileInZip userInfo:@{NSLocalizedDescriptionKey: @"failed to open file in zip file"}];
                success = NO;
                break;
            }
            
            // Reading data and write to file
            kk_unz_file_info fileInfo;
            memset(&fileInfo, 0, sizeof(kk_unz_file_info));
            
            ret = unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:@"KKSSZipArchiveErrorDomain" code:SSZipArchiveErrorCodeFileInfoNotLoadable userInfo:@{NSLocalizedDescriptionKey: @"failed to retrieve info for file"}];
                success = NO;
                unzCloseCurrentFile(zip);
                break;
            }
            ...
            if ([delegate respondsToSelector:@selector(zipArchiveShouldUnzipFileAtIndex:totalFiles:archivePath:fileInfo:)]) {
                if (![delegate zipArchiveShouldUnzipFileAtIndex:currentFileNumber
                                                     totalFiles:(NSInteger)configInfo.number_entry
            ...
            if ([delegate respondsToSelector:@selector(zipArchiveWillUnzipFileAtIndex:totalFiles:archivePath:fileInfo:)]) {
                [delegate zipArchiveWillUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)configInfo.number_entry
                                             archivePath:path fileInfo:fileInfo];
            }
            if ([delegate respondsToSelector:@selector(zipArchiveProgressEvent:total:)]) {
            ...
            if (nil != err) {
                ...
                NSLog(@"[KKSSZipArchive] Error: %@", err.localizedDescription);
            }
                        ...
                        if (readBytes > 0) {
                            if (0 == fwrite(buffer, readBytes, 1, fp)) {
                                if (ferror(fp)) {
                                    ...
                                    NSLog(@"[KKSSZipArchive] %@", message);
                                    success = NO;
                                    unzippingError = [NSError errorWithDomain:@"KKSSZipArchiveErrorDomain" code:SSZipArchiveErrorCodeFailedToWriteFile userInfo:@{NSLocalizedDescriptionKey: message}];
                                    break;
                                }
                            }
                        } else {
                        ...
                        if (preserveAttributes) {
                            ...
                            if (fileInfo.dos_date != 0) {
                                ...
                                if (attr) {
                                    if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                                        ...
                                        NSLog(@"[KKSSZipArchive] Failed to set attributes - whilst setting modification date");
                                    }
                                }
                            }
                            ...
                            if (permissions != 0) {
                                ...
                                if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                                    ...
                                    NSLog(@"[KKSSZipArchive] Failed to set attributes - whilst setting permissions");
                                }
                            }
                        }
                    }
                ...
                if (overwrite)
                {
                    if ([fileManager fileExistsAtPath:fullPath])
                    {
                        ...
                        if (!success)
                        {
                            ...
                            NSLog(@"[KKSSZipArchive] %@", message);
                            success = NO;
                            unzippingError = [NSError errorWithDomain:KKSSZipArchiveErrorDomain code:error.code userInfo:@{NSLocalizedDescriptionKey: message}];
                        }
                    }
                }
            ...
            if ([delegate respondsToSelector:@selector(zipArchiveDidUnzipFileAtIndex:totalFiles:archivePath:fileInfo:)]) {
                [delegate zipArchiveDidUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)configInfo.number_entry
                                            archivePath:path fileInfo:fileInfo];
            ...
            if (progressHandler)
            {
                progressHandler(strPath, fileInfo, currentFileNumber, configInfo.number_entry);
            }
        }
    ...
    if (success && preserveAttributes) {
        ...
        for (NSDictionary * d in directoriesModificationDates) {
            if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: d[@"modDate"]} ofItemAtPath:d[@"path"] error:&err]) {
                NSLog(@"[KKSSZipArchive] Set attributes failed for directory: %@.", d[@"path"]);
            }
            if (err) {
                NSLog(@"[KKSSZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
            }
        }
    }
    ...
    if (success && [delegate respondsToSelector:@selector(zipArchiveDidUnzipArchiveAtPath:zipInfo:unzippedPath:)]) {
        [delegate zipArchiveDidUnzipArchiveAtPath:path zipInfo:configInfo unzippedPath:destination];
    }
    ...
    if (crc_ret == UNZ_CRCERROR)
    {
        ...
        retErr = [NSError errorWithDomain:KKSSZipArchiveErrorDomain code:SSZipArchiveErrorCodeFileInfoNotLoadable userInfo:userInfo];
    }
...
{
    return [KKSSZipArchive createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil];
}
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath{
    return [KKSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory{
    return [KKSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths withPassword:(NSString *)password
{
    BOOL success = NO;
    KKSSZipArchive *zipArchive = [[KKSSZipArchive alloc] initWithPath:path];
    if ([zipArchive open]) {
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath withPassword:(nullable NSString *)password{
    return [KKSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password];
}


+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password{
    return [KKSSZipArchive createZipFileAtPath:path
...
- (BOOL)close
{
    NSAssert((_zip != NULL), @"[KKSSZipArchive] Attempting to close an archive which was never opened");
    zipClose(_zip, NULL);
    return YES;
}
...
// Two consecutive words, or a longword, KKYYYYYYYMMMMDDDDD hhhhhmmmmmmsssss
// KKYYYYYYY is years from 1980 = 0
// sssss is (seconds/2).
//
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[KKSSZipArchive] MSDOS date masks don't add up");
