...
typedef struct
{
    ...
    kk_unz_global_info64 gi;               /* public global information */
    ...
    kk_unz_file_info64 cur_file_info;      /* public info about the current file in zip*/
...
extern int ZEXPORT unzGetGlobalInfo(unzFile file, kk_unz_global_info* pglobal_info32)
{
    unz64_s *s = NULL;
    if (file == NULL)
        return UNZ_PARAMERROR;
...
extern int ZEXPORT unzGetGlobalInfo64(unzFile file, kk_unz_global_info64 *pglobal_info)
{
    unz64_s *s = NULL;
    if (file == NULL)
        return UNZ_PARAMERROR;
...
static int unzGetCurrentFileInfoInternal(unzFile file, kk_unz_file_info64 *pfile_info,
...
extern int ZEXPORT unzGetCurrentFileInfo(unzFile file, kk_unz_file_info *pfile_info, char *filename,
    uint16_t filename_size, void *extrafield, uint16_t extrafield_size, char *comment, uint16_t comment_size)
{
    kk_unz_file_info64 file_info64;
    int err = UNZ_OK;
...
extern int ZEXPORT unzGetCurrentFileInfo64(unzFile file, kk_unz_file_info64 * pfile_info, char *filename,
    uint16_t filename_size, void *extrafield, uint16_t extrafield_size, char *comment, uint16_t comment_size)
{
...
extern int ZEXPORT unzGoToFirstFile2(unzFile file, kk_unz_file_info64 *pfile_info, char *filename,
    uint16_t filename_size, void *extrafield, uint16_t extrafield_size, char *comment, uint16_t comment_size)
{
    unz64_s *s = NULL;
    int err = UNZ_OK;
    ...
    if ((err == UNZ_OK) && (pfile_info != NULL))
        memcpy(pfile_info, &s->cur_file_info, sizeof(kk_unz_file_info64));
...
extern int ZEXPORT unzGoToNextFile2(unzFile file, kk_unz_file_info64 *pfile_info, char *filename,
    uint16_t filename_size, void *extrafield, uint16_t extrafield_size, char *comment, uint16_t comment_size)
{
    unz64_s *s = NULL;
    int err = UNZ_OK;
...
{
    unz64_s *s = NULL;
    kk_unz_file_info64 cur_file_info_saved;
